            ...
            child: ConstrainedBox(
              ...
              child: ListView(
                ...
                children: <Widget>[
                  ...
                  SegmentedButton<String>(
                    segments: const <ButtonSegment<String>>[
                      ButtonSegment<String>(
                        value: 'Collector',
                        label: Text('Collector view'),
                        icon: Icon(Icons.local_shipping_outlined),
                      ),
                      ButtonSegment<String>(
                        value: 'Owner',
                        label: Text('Owner view'),
                  ...
                  if (_tab == 0) ...<Widget>[
                    Row(children: <Widget>[
                      Expanded(
                        ...
                        label: 'In progress',
                        value: demoPickups
                            .where((pickup) => collectionMilestones
                                .take(4)
                      ...
                      Expanded(
                        ...
                        value: demoPickups
                            .where((pickup) => pickup.status == 'Failed')
                            .length
                            .toString(),
                    ...
                    _Surface(
                      ...
                      child: Column(
                          ...
                          children: <Widget>[
                            ...
                            const Text('The chair’s next chapter.',
                                style: TextStyle(
                            ...
                            const Text(
                                ...
                                style: TextStyle(
                                    color: Color(0xFF84652E), fontSize: 12)),
                    ...
                    ...demoPickups
                        ...
                        .map((pickup) => _PickupCard(
                            pickup: pickup, onTap: () => _openPickup(pickup))),
                    const SizedBox(height: 8),
                    _Surface(
                      ...
                      child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: <Widget>[
                            const Text('A collection needs a new plan',
                                style: TextStyle(fontWeight: FontWeight.w600)),
                            const SizedBox(height: 7),
                            const Text(
                                'Small refrigerator · assigned vehicle unavailable.',
                                style: TextStyle(color: _muted, fontSize: 12)),
                            TextButton(
                                onPressed: () => _openPickup(demoPickups[2]),
                  ...
                  if (_tab == 1) ...<Widget>[
                    TextField(
                      ...
                      decoration: const InputDecoration(
                          labelText: 'Search item or pickup ID',
                    ...
                    if (visible.isEmpty)
                      const _Surface(
                          child: Text('No demo pickups match your search.')),
                    ...visible.map((pickup) => _PickupCard(
                        pickup: pickup, onTap: () => _openPickup(pickup))),
                  ],
                  if (_tab == 2)
                    _Surface(
                      child: Column(
                          ...
                          children: <Widget>[
                            ...
                            const Text(
                                'Choose a preferred date and time window. The backend will need to check slots and destination hours.',
                                style: TextStyle(color: _muted)),
                            const SizedBox(height: 18),
                            if (_availabilityDate != null) ...<Widget>[
                              Text(
                                  'Local preference: ${_availabilityDate!.day}/${_availabilityDate!.month}/${_availabilityDate!.year}',
                                  style: const TextStyle(
                                      fontWeight: FontWeight.w600)),
                              Text(
                                  '${_availabilityStart!.format(context)} – ${_availabilityEnd!.format(context)}'),
                              const SizedBox(height: 15),
                            ],
                            FilledButton.icon(
                                onPressed: _availability,
                                icon: const Icon(Icons.calendar_month_outlined),
                                label: Text(_availabilityDate == null
                                    ? 'Select availability'
                                    : 'Edit availability')),
                  ...
                  const Text('Member 4 · Pickup & handover UI preview',
        ...
        bottomNavigationBar: NavigationBar(
          ...
          destinations: const <NavigationDestination>[
            NavigationDestination(
                icon: Icon(Icons.grid_view_outlined), label: 'Overview'),
            NavigationDestination(
                icon: Icon(Icons.inventory_2_outlined), label: 'Pickups'),
            NavigationDestination(
...
class _PickupDetailState extends State<_PickupDetail> {
  ...
  Future<void> _reportFailure() async {
    ...
    widget.onNote(note);
    _message(
        'Failure note saved locally. No re-planning workflow was started.');
  }
  ...
  void _recordDecision(String action) {
    ...
    _message(
        '$action preference recorded locally. Only authorized staff and the backend can confirm a booking.');
  }
            ...
            child: ConstrainedBox(
              ...
              child: ListView(
                  ...
                  children: <Widget>[
                    ...
                    Text(pickup.item,
                        style: const TextStyle(
                            fontSize: 30, fontFamily: 'serif', color: _ink)),
                    const SizedBox(height: 10),
                    Align(
                        alignment: Alignment.centerLeft,
                        child: _Status(pickup.status)),
                    const SizedBox(height: 20),
                    _Surface(
                        child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                          ...
                          _DetailLine(Icons.schedule_outlined,
                              'Collection window', pickup.window),
                          _DetailLine(Icons.place_outlined, 'Pickup location',
                              pickup.address),
                          _DetailLine(Icons.storefront_outlined, 'Destination',
                              pickup.destination),
                          _DetailLine(Icons.local_shipping_outlined, 'Vehicle',
                              pickup.vehicle),
                          _DetailLine(Icons.inventory_2_outlined, 'Handling',
                              pickup.handling),
                          const SizedBox(height: 8),
                          const Text(
                              'Map and travel estimate unavailable. No routing service is connected.',
                              style: TextStyle(color: _muted, fontSize: 12)),
                        ])),
                    const SizedBox(height: 20),
                    if (pickup.status == 'Awaiting review') ...<Widget>[
                      const _Surface(
                        ...
                        child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: <Widget>[
                              _Eyebrow('COLLECTION PROPOSAL · STATIC EXAMPLE'),
                              SizedBox(height: 12),
                              _SectionTitle('Friday, 14:00–16:00'),
                              Text(
                                  'Small van · 50 kg sample capacity\nOwner available 14:00–17:00\nDestination open 09:00–17:00',
                                  style: TextStyle(height: 1.8)),
                              SizedBox(height: 15),
                              Text('Why this sample option?',
                                  style:
                                      TextStyle(fontWeight: FontWeight.w600)),
                              SizedBox(height: 6),
                              Text(
                                  'The listed capacity exceeds 8 kg and the time windows overlap. Vehicle dimensions, handling support, and actual availability are unvalidated.',
                                  style:
                                      TextStyle(color: _muted, fontSize: 12)),
                              SizedBox(height: 15),
                              Text(
                                  'Travel distance, duration, and cost: unavailable. Feasibility: manual review required.',
                                  style: TextStyle(
                                      color: Color(0xFF84652E), fontSize: 12)),
                              Divider(height: 28),
                              Text('Fallback: 16:00–18:00',
                                  style:
                                      TextStyle(fontWeight: FontWeight.w600)),
                              SizedBox(height: 6),
                              Text(
                                  'Destination closes at 17:00. This option needs revision; arrival time is unverified.',
                                  style:
                                      TextStyle(color: _muted, fontSize: 12)),
                            ]),
                      ),
                      ...
                      _Surface(
                          child: Column(
                            ...
                            const Text(
                                'These controls save a UI preference. The plan stays unconfirmed; no agent has run.',
                                style: TextStyle(color: _muted, fontSize: 12)),
                            const SizedBox(height: 16),
                            TextField(
                                controller: _review,
                            ...
                            Wrap(spacing: 8, runSpacing: 8, children: <Widget>[
                              FilledButton(
                                  onPressed: () => _recordDecision('Approve'),
                                  child: const Text('Approve · preview')),
                              OutlinedButton(
                                  onPressed: () => _recordDecision('Reject'),
                                  child: const Text('Reject · preview')),
                              OutlinedButton(
                                  onPressed: () =>
                                      _recordDecision('Request Revision'),
                                  child:
                                      const Text('Request Revision · preview')),
                            ]),
                            if (_decision != null)
                              Padding(
                                  padding: const EdgeInsets.only(top: 15),
                                  child: Text(
                                      'Local preference: $_decision\nBooking remains unconfirmed.',
                                      style: const TextStyle(
                        ...
                        child: Column(
                          ...
                          if (stepIndex < 0)
                            Padding(
                                ...
                                child: Text(
                                    'Current status: ${pickup.status}. No completed milestones are inferred.',
                                    style: const TextStyle(
                          ...
                          if (widget.role == 'Collector' &&
                            ...
                            DropdownButtonFormField<String>(
                              ...
                              isExpanded: true,
                              decoration: const InputDecoration(
                                  labelText: 'Preview a status update'),
                              items: collectionMilestones
                                  ...
                                  .toList(),
                              onChanged: (value) =>
                                  setState(() => _milestonePreview = value),
                            ),
                            const SizedBox(height: 12),
                            OutlinedButton(
                              onPressed: _milestonePreview == null
                                  ? null
                                  : () => _message(
                                      'Preview request: $_milestonePreview. Backend validation is unavailable; the recorded status has not changed.'),
                      ...
                      _Surface(
                          ...
                          child: Column(
                              ...
                              children: <Widget>[
                                ...
                                Text(_note ??
                                    'Assigned vehicle became unavailable.'),
                                const SizedBox(height: 10),
                                const Text(
                                    'Re-planning must recheck slots, capacity, handling, availability, destination hours, and travel before requesting human approval.',
                                    style:
                                        TextStyle(color: _muted, fontSize: 12)),
                                const SizedBox(height: 8),
                                const Text(
                                    'No revised plan generated. Local notes are not a persistent audit trail.',
                                    style:
                                        TextStyle(color: _muted, fontSize: 12)),
                              ])),
                      const SizedBox(height: 20),
                    ...
                    if (pickup.status != 'Handover Verified') ...<Widget>[
                      OutlinedButton.icon(
                          ...
                          label: const Text('Draft failed-collection report')),
                      const SizedBox(height: 20),
                    ],
                    _Surface(
                        child: Form(
                      ...
                      child: Column(
                          ...
                          children: <Widget>[
                            ...
                            const Text(
                                'Code validation and proof submission require the ASP.NET backend.',
                                style: TextStyle(color: _muted, fontSize: 12)),
                            const SizedBox(height: 18),
                            const Row(children: <Widget>[
                              ...
                              Expanded(
                                  child: Text(
                                      'QR scanner unavailable\nCamera integration is not connected.',
                                      style: TextStyle(
                                          color: _muted, fontSize: 12))),
                            ]),
                            const SizedBox(height: 20),
                            TextFormField(
                              ...
                              maxLength: 6,
                              decoration: const InputDecoration(
                                  labelText: 'One-time code (demo: 6 digits)'),
                              validator: (value) => RegExp(r'^\d{6}$')
                                      .hasMatch(value ?? '')
                                  ? null
                                  : 'Enter exactly 6 digits for the preview.',
                            ),
                            TextFormField(
                                controller: _proof,
                                maxLines: 2,
                                ...
                                decoration: const InputDecoration(
                                    labelText:
                                        'Handover proof note (local preview)')),
                            const Text(
                                'Photo capture and upload are not connected.',
                                style: TextStyle(color: _muted, fontSize: 11)),
                            const SizedBox(height: 16),
                            FilledButton.icon(
                              onPressed: () {
                                if (_handoverForm.currentState!.validate()) {
                                  _message(
                                      'Code format accepted for preview only. No code was verified, proof uploaded, or handover status changed.');
                                }
                              },
                    ...
                    const SizedBox(height: 24),
                    const Text(
                        'Demo changes stay on this device and do not sync to React.',
                        style: TextStyle(color: _muted, fontSize: 11),
                        textAlign: TextAlign.center),
                  ]),
            ),
...
class _AvailabilityDialog extends StatefulWidget {
  const _AvailabilityDialog(
      {this.initialDate, this.initialStart, this.initialEnd});
  ...
  Widget build(BuildContext context) => AlertDialog(
        ...
        content: SingleChildScrollView(
            child: Column(
                ...
                children: <Widget>[
              const Text(
                  'Save a preferred window locally. This does not book a slot.'),
              const SizedBox(height: 16),
              OutlinedButton.icon(
                onPressed: () async {
  ...
  Widget build(BuildContext context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: <Widget>[
            const Text(
                'Describe the changed constraint. This note stays in the UI session and does not start an agent.'),
            const SizedBox(height: 16),
            TextFormField(
                controller: _reason,
                maxLines: 4,
                maxLength: 500,
                decoration: const InputDecoration(
                    labelText: 'Reason / changed constraint'),
...
class _PreviewBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
            color: const Color(0xFFE7ECDE),
            borderRadius: BorderRadius.circular(8)),
        child: const Text(
            'UI PREVIEW · Fictional data\nSession only. No bookings, AI execution, or verification.',
            style:
                TextStyle(fontSize: 11, color: Color(0xFF586C48), height: 1.5)),
      );
}
  ...
  Widget build(BuildContext context) => _Surface(
          child: Column(
            ...
            Text(value.padLeft(2, '0'),
                style: const TextStyle(fontSize: 30, color: _ink)),
...
class _Eyebrow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Text(text,
      style: const TextStyle(fontSize: 9, letterSpacing: 1.4, color: _muted));
}
...
class _Status extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
            color: status == 'Failed'
                ? const Color(0xFFFBEDE8)
                : const Color(0xFFEAF0E3),
            borderRadius: BorderRadius.circular(5)),
        child: Text(status,
            style: TextStyle(
                fontSize: 11,
                color: status == 'Failed' ? const Color(0xFFA15D41) : _forest)),
      );
}
                ...
                child: Column(
                    ...
                    children: <Widget>[
                      Row(children: <Widget>[
                        ...
                        const SizedBox(width: 12),
                        Expanded(
                            child: Text(pickup.item,
                                style: const TextStyle(
                                    fontWeight: FontWeight.w600, color: _ink))),
                        const Icon(Icons.chevron_right, color: _muted),
                      ]),
                      const SizedBox(height: 10),
                      Text('${pickup.id} · ${pickup.window}',
