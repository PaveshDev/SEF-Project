...
import '../services/collections_identity.dart';
...
class PickupDetailScreen extends StatefulWidget {
  ...
  final CollectionsActorProvider? actorProvider;
...
class _PickupDetailScreenState extends State<PickupDetailScreen> {
  ...
  Future<void> _submitHandoverVerification() async {
    ...
    try {
      final actorId = await requireCollectionsActorId(widget.actorProvider);
      if (!mounted) return;
      final result = await _api.verifyHandoverCode(
          widget.pickup.id, _code.text.trim(), actorId);
      if (!mounted) return;
      if (result != null) {
      ...
      if (mounted) {
        setState(() => _errorMessage = e is StateError
            ? e.message
                  ...
                  children: <Widget>[
                    Text(pickup.item,
                        style: const TextStyle(
                            fontSize: 28, fontFamily: 'serif', color: _ink)),
                    ...
                    _Card(
                        child: Column(
                          ...
                          _DetailRow(
                              Icons.schedule_outlined, 'Window', pickup.window),
                          _DetailRow(
                              Icons.place_outlined, 'Location', pickup.address),
                          _DetailRow(Icons.storefront_outlined, 'Destination',
                              pickup.destination),
                          _DetailRow(Icons.local_shipping_outlined, 'Vehicle',
                              pickup.vehicle),
                          _DetailRow(Icons.inventory_2_outlined, 'Handling',
                              pickup.handling),
                        ])),
                    const SizedBox(height: 20),
                    ...
                    _Card(
                        child: Column(
                          ...
                          Row(
                              ...
                              children: <Widget>[
                                const Text('Collection milestones',
                                    style: TextStyle(
                                        fontSize: 17,
                                        fontWeight: FontWeight.w600,
                                        color: _ink)),
                    ...
                    if (_events.isNotEmpty) ...<Widget>[
                      _Card(
                          child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: <Widget>[
                            const Text('Backend Audit Event Logs',
                                style: TextStyle(
                                    fontSize: 17,
                                    fontWeight: FontWeight.w600,
                                    color: _ink)),
                    ...
                    _Card(
                      ...
                      child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: <Widget>[
                            const Text('Handover Verification (OTP)',
                                style: TextStyle(
                                    fontSize: 17,
                                    fontWeight: FontWeight.w600,
                                    color: _ink)),
                            ...
                            Text(
                                widget.actorProvider == null
                                    ? collectionsIdentityUnavailable
                                    : 'Verification submits to the backend API for authorization.',
                                style: const TextStyle(
                                    color: _muted, fontSize: 12)),
                            const SizedBox(height: 18),
                            Row(children: <Widget>[
                              const Icon(Icons.qr_code_scanner,
                                  size: 32, color: _forest),
                              const SizedBox(width: 12),
                              Expanded(
                                    ...
                                    final refreshed =
                                      ...
                                      MaterialPageRoute<bool>(
                                        builder: (context) => HandoverScreen(
                                            pickupId: pickup.id,
                                            itemName: pickup.item,
                                            apiService: _api,
                                            actorProvider:
                                                widget.actorProvider),
                                      ),
                                    );
                            ...
                            TextFormField(
                              ...
                              decoration: const InputDecoration(
                                  labelText: 'One-time code (6 digits)'),
                              validator: (value) =>
                            ...
                            FilledButton.icon(
                              onPressed: (widget.actorProvider == null ||
                                      _verifyingCode)
                                  ? null
                                  : _submitHandoverVerification,
                              icon: _verifyingCode
                                  ? const SizedBox(
                                      width: 16,
                                      height: 16,
                              ...
                              label: Text(_verifyingCode
                                  ? 'Verifying...'
                                  : 'Submit OTP Code to Backend'),
                            ),
                          ]),
                    )),
                    const SizedBox(height: 24),
                    const Text('Member 4 · Synchronized with ASP.NET backend',
        ...
        child: Container(
          ...
          child: Text(status,
              style: TextStyle(
                  fontSize: 11,
                  color:
                      status == 'Failed' ? const Color(0xFFA15D41) : _forest)),
        ),
      );
}
