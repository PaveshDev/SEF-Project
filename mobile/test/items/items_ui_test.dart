...
void main() {
  ...
  Widget createRouterApp(String initialLocation) {
    final router = GoRouter(
      ...
      routes: [
        ...
        GoRoute(
            path: '/items', builder: (context, state) => const MyItemsScreen()),
        GoRoute(
            path: '/items/create',
        ...
        GoRoute(
            path: '/items/:id/condition-answers',
            builder: (context, state) =>
                ConditionAnswersScreen(itemId: state.pathParameters['id']!)),
        GoRoute(
            path: '/items/:id/assessments/:assessmentId',
            builder: (context, state) => AssessmentScreen(
                itemId: state.pathParameters['id']!,
                assessmentId: state.pathParameters['assessmentId']!)),
        GoRoute(
            path: '/items/:id/clarifications/:clarificationId',
            builder: (context, state) => ClarificationScreen(
                itemId: state.pathParameters['id']!,
                clarificationId: state.pathParameters['clarificationId']!)),
        GoRoute(
            path: '/items/:id/assessments/:assessmentId/confirm',
            builder: (context, state) => ConfirmAssessmentScreen(
                itemId: state.pathParameters['id']!,
                assessmentId: state.pathParameters['assessmentId']!)),
        GoRoute(
            path: '/items/:id/assessments/:assessmentId/reassessment',
            builder: (context, state) => ReassessmentScreen(
                itemId: state.pathParameters['id']!,
                assessmentId: state.pathParameters['assessmentId']!)),
      ],
    );
  ...
  testWidgets('1. Item list displays returned items',
  ...
  testWidgets('2. Empty item list displays empty state',
      (WidgetTester tester) async {
    apiClient.httpClientAdapter = MockDioAdapter((options) async {
      if (options.path == '/api/items') {
  ...
  testWidgets('3. Create item form submits correct request',
      (WidgetTester tester) async {
  ...
  testWidgets('4. Item details display correctly', (WidgetTester tester) async {
    ...
    await tester
        .pumpWidget(createTestApp(const ItemDetailsScreen(itemId: '1')));
    await tester.pumpAndSettle();
  ...
  testWidgets('5. 400 validation error is displayed',
      (WidgetTester tester) async {
    apiClient.httpClientAdapter = MockDioAdapter((options) async {
      if (options.path == '/api/items' && options.method == 'POST') {
        return ResponseBody.fromString('Title too short', 400, headers: {
  ...
  testWidgets('6. 401 authentication error is handled',
      (WidgetTester tester) async {
    apiClient.httpClientAdapter = MockDioAdapter((options) async {
      if (options.path == '/api/items') {
  ...
  testWidgets('7. 409 concurrency conflict is displayed',
      (WidgetTester tester) async {
    apiClient.httpClientAdapter = MockDioAdapter((options) async {
  ...
  testWidgets('8. Condition answers submit correctly',
      (WidgetTester tester) async {
    bool apiCalled = false;
    apiClient.httpClientAdapter = MockDioAdapter((options) async {
      if (options.path == '/api/items/1/condition-answers' &&
          options.method == 'POST') {
        apiCalled = true;
    ...
    expect(apiCalled, true);
    expect(
        find.text('Condition answers submitted successfully'), findsOneWidget);
  });

  testWidgets('9. Submit-for-assessment action calls the correct endpoint',
      (WidgetTester tester) async {
    bool apiCalled = false;
    apiClient.httpClientAdapter = MockDioAdapter((options) async {
      if (options.path == '/api/items/1') {
  ...
  testWidgets('10. Assessment displays AI and owner information separately',
      (WidgetTester tester) async {
    apiClient.httpClientAdapter = MockDioAdapter((options) async {
      if (options.path == '/api/items/1/assessments/a1') {
        final data = {
          ...
          'visibleObservations': 'None',
          'ownerReportedFunctionality': 'Working fine',
          'confidence': 0.95,
    ...
    await tester.pumpWidget(
        createTestApp(const AssessmentScreen(itemId: '1', assessmentId: 'a1')));
    await tester.pumpAndSettle();
  ...
  testWidgets('11. Clarification answer submits correctly',
      (WidgetTester tester) async {
    bool apiCalled = false;
    apiClient.httpClientAdapter = MockDioAdapter((options) async {
      if (options.path == '/api/items/1/clarifications/c1/answer' &&
          options.method == 'POST') {
        apiCalled = true;
        return ResponseBody.fromString('', 200);
      }
  ...
  testWidgets('12. Assessment confirmation requires explicit interaction',
      (WidgetTester tester) async {
    bool apiCalled = false;
    apiClient.httpClientAdapter = MockDioAdapter((options) async {
      if (options.path == '/api/items/1/assessments/a1/confirm' &&
          options.method == 'POST') {
        apiCalled = true;
        return ResponseBody.fromString('', 200);
      }
  ...
  testWidgets('13. Reassessment requires a reason',
      (WidgetTester tester) async {
    bool apiCalled = false;
    apiClient.httpClientAdapter = MockDioAdapter((options) async {
      if (options.path == '/api/items/1/assessments/a1/reassessment' &&
          options.method == 'POST') {
        apiCalled = true;
        return ResponseBody.fromString('', 200);
      }
    ...
    await tester
        .pumpWidget(createRouterApp('/items/1/assessments/a1/reassessment'));
    await tester.pumpAndSettle();
