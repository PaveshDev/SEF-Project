...
public sealed class PickupRequestService : IPickupRequestService
{
    ...
    {
        // Every pickup must reference an existing, available collection slot.
        if (request.CollectionSlotId == Guid.Empty)
        {
            throw new InvalidOperationException("CollectionSlotId is required.");
        }

        var slot = await _db.CollectionSlots.FirstOrDefaultAsync(s => s.Id == request.CollectionSlotId, ct);
        if (slot is null)
        {
    ...
    public async Task<PickupRequestReadDto?> UpdateAsync(Guid id, UpdatePickupRequestRequest request, CancellationToken ct = default)
    {
            ...
            !request.Status.Equals(entity.Status, StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                var hasProof = await _db.HandoverProofs.AnyAsync(h =>
                    h.PickupRequestId == id &&
                    h.VerifiedBy != null && h.VerifiedBy != Guid.Empty &&
                    h.VerifiedAt != null, ct);
                if (!hasProof)
                {
