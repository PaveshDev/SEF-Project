...
public interface IRecoveryPlanningService
{
    ...
    Task<PagedResponse<RecoveryCaseResponse>> ListAsync(RecoveryCaseQuery query, CancellationToken ct);
    ...
    Task DeleteAsync(Guid id, string key, CancellationToken ct);
    Task<PlanningResponse> ReplanAsync(Guid id, StartPlanningRequest request, string key, CancellationToken ct);
}
