async (livePage) => {
 ...
 const dir='C:/Users/paves/Downloads/PRo/docs/members/member-2/screenshots-2026-09-09/';
 ...
 const estimate={valueLow:4000,valueHigh:6000,repairCost:0,pickupCost:0,netValue:4000,shortfall:0,currency:'LKR'};
 ...
 await page.getByLabel('Proposal expiry',{exact:true}).fill('2026-10-01T10:00');
 ...
 await page.getByRole('button',{name:'Open planning'}).click();
 await page.getByText('Create a proposal from a selected option to review it here.',{exact:true}).waitFor();
 checks.push({test:'existing proposal survives leaving/reopening',pass:false,reopenRequests:requests.slice(before)});
 await capture('reopen-missing-proposal');
 await page.close();
